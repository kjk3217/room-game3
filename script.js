...
function openQuiz(quizId) {
    ...
    document.getElementById('quizTitle').style.display = 'block'; // ✨ none -> block으로 수정
...
function createQuizInput(type) {
   ...
   if (type === 'single' || type === 'password') {
       ...
       for (let i = 1; i <= 2; i++) { // ✨ 4 -> 2로 수정 (2개 입력칸만 생성)
           const input = document.createElement('input');
           input.type = 'text';
           input.id = \`answer\${i}\`;
...
function showRoom(roomNum) {
   for (let i = 1; i <= 3; i++) {
       ...
       room.classList.remove('active', 'exit-left'); // ✨ display:none 대신 class로 컨트롤
   }
...
function showMessage(text) {
   ...
   // ✨ 스타일시트에서 스타일을 관리하도록 CSS 클래스만 추가
   document.body.appendChild(message);
